...
export function AddStoryModal({ open, onOpenChange, sprintId, onAddStory, settings, onSave }: AddStoryModalProps) {
  ...
  useEffect(() => {
    if (open) {
      ...
      setTagInput("")
      // Focus on prompt field for AI workflow
      setTimeout(() => {
    ...
    <div className="fixed inset-0 bg-[rgba(28,28,28,0.5)] flex items-center justify-center z-50 p-5">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-[580px] max-h-[95vh] overflow-y-auto">
        ...
        <div className="px-6 py-4 border-b border-[#E0E0E0] relative">
          <h1 className="text-2xl font-bold text-[#1C1C1C] mb-0">Add New Story</h1>
          <button
            onClick={handleClose}
