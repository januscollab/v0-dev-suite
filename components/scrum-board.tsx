...
import { useHybridScrumDataEnhanced } from "@/hooks/use-hybrid-scrum-data-enhanced"
...
import { setArchivedSprints } from "@/utils/setArchivedSprints" // Import or declare the variable here
...
export function ScrumBoard() {
  ...
  const handleCloseCompleted = (sprintId: string) => {
    ...
    // Archive completed stories by moving them to archived sprints
    if (completedStories.length > 0) {
      const archivedSprint: Sprint = {
        ...sprint,
        id: `archived-${sprint.id}-${Date.now()}`,
        name: `${sprint.name} (Completed Stories)`,
        stories: completedStories,
        isActive: false,
      }

      setArchivedSprints((prev) => [...prev, archivedSprint])

      // Remove completed stories from active sprint
  ...
  const handleCloseAll = (sprintId: string) => {
    ...
    // Mark all stories as completed and archive the entire sprint
    ...
    setArchivedSprints((prev) => [...prev, updatedSprint])
  ...
  const handleDragStart = (event: DragStartEvent) => {
    ...
    const sprint = sprints?.find((sprint) => sprint.id === active.id)
    if (sprint) {
